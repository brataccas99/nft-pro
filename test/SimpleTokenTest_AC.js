...
const JewelCoin = artifacts.require('JewelCoin');

const ROLE = web3.utils.sha3('MINTER_ROLE');
// Start test block
contract('JewelCoin', function ([ creator, minter, other ]) {
  const NAME = 'JewelCoin';
  const SYMBOL = 'HC';
  const TOTAL_SUPPLY = new BN('10000000000000000000000');

  beforeEach(async function () {
    this.token = await JewelCoin.new(NAME, SYMBOL, TOTAL_SUPPLY, { from: creator });
  });
  ...
  it('should return the correct item ID for the first item of the CryptoGym', async function () {
    ...
    const result = await this.token.payItem.call(price, itemId, { from: creator });
